...
def load_data():
    ...
    # 1. DFDC
    dfdc_folders = glob(os.getcwd() + "/data/dfdc/dfdc*")
    ...
    celeb_df["path"] = celeb_df["Unnamed: 0"].apply(
        lambda x: os.path.join("./data/CelebDF-v2/faces", x.split(".")[0])
    )
    ...
    ytf_df["path"] = ytf_df["videoID"].apply(
        lambda x: os.path.join("./data/youtube_faces_dataset/faces", x)
    )
    ytf_df.head()
